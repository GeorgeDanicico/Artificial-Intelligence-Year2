...
class Environment:
    ...
    def readUDMSensors(self, x, y):
        ...
        while (xf >= 0) and (self.__surface[xf][y] == 0):
            xf = xf - 1
            readings[utils.UP] = readings[utils.UP] + 1
        # DOWN
        xf = x + 1
        while (xf < self.__n) and (self.__surface[xf][y] == 0):
            xf = xf + 1
            readings[utils.DOWN] = readings[utils.DOWN] + 1
        # LEFT
        yf = y + 1
        while (yf < self.__m) and (self.__surface[x][yf] == 0):
            yf = yf + 1
            readings[utils.LEFT] = readings[utils.LEFT] + 1
        # RIGHT
        yf = y - 1
        while (yf >= 0) and (self.__surface[x][yf] == 0):
            yf = yf - 1
            readings[utils.RIGHT] = readings[utils.RIGHT] + 1
